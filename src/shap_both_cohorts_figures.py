...
2. BOTH COHORTS. SHAP is computed on the held-out UMM test partition (n=28)
   and on the external MAINZ cohort (n=284). See the note below on what that
...
Statistical note: the UMM values rest on 28 patients, the MAINZ values on 284.
The +/- SD in the tables is the spread across the m=10 ensemble members, not
sampling uncertainty -- with n=28 the latter dominates. The external estimates
...
BEST_MODEL_PER_TASK = {
    'UMM':   {'fibrosis': 'xgb', 'two_stage': 'xgb',
              'cirrhosis': 'light_gbm', 'three_stage': 'xgb'},
    'MAINZ': {'fibrosis': 'xgb', 'two_stage': 'xgb',
              'cirrhosis': 'xgb', 'three_stage': 'xgb'},
}
...
# axes and n=28 internally, the bands overlap into an unreadable wash. The same
...
def roc_panel_combined(cohort, entries, stem):
    ...
    overlapping bands in the ROC area cover most of the plot at n=28 and carry
...
def main():
    ...
    if table_rows:
        ...
        lines = [r'\begin{table*}[htbp]', r'    \centering',
                 ...
                 r'    block gives the top five on the held-out UMM test partition (n=28), the',
                 r'    right block the top five on the external MAINZ cohort (n=284), each by the',
